...
const Leaderboard = ({ loading, sessions, players }) => {
  ...
  */
  ...
  const netGains = players.reduce(
    (acc, p) => acc + (p.historicalBalance < 0 ? 0 : p.historicalBalance),
    0
  );
  const totalBuyins = players.reduce((acc, p) => acc + p.totalBuyin, 0);
  const largestSess = sessions.reduce(
    (max, p) => (p.playerSessions.length > max ? p.playerSessions.length : max),
    0
  );
    ...
    <div className={css.leaderboard}>
      ...
      {!loading && (
        <div className={css.wrapper}>
          <Champs champs={top3} title="TOP 3" type="top" />

          <Table people={players} />
